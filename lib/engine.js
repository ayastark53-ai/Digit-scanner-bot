...
export class TradingEngine {
  constructor({ client, symbols, settings, onEvent }) {
    ...
    this.scanning = false; // ticks are flowing and being scored
    this.armed = false; // whether a fired signal should actually place a trade
    this.inTrade = false;
  ...
  // Begin scanning markets. Safe to call once, right after connecting —
  // this does NOT place trades on its own, it only watches and reports.
  async startScanning() {
  ...
  stopScanning(reason) {
    ...
    this.onEvent("status", { message: reason || "Scanning stopped." });
  }

  // Arm/disarm live trading without touching the scanner subscriptions.
  setArmed(armed) {
    this.armed = armed;
    if (armed) {
      this.sessionPnl = 0;
      ...
      this.martingaleStep = 0;
      this.winStreak = 0;
      this.onEvent("status", { message: "Bot armed — it will trade automatically when a setup fires." });
    } else {
      this.onEvent("status", { message: "Bot disarmed. Still scanning, but won't place trades." });
    }
  }
  ...
  _onTick(symbol, tick) {
    ...
    if (result.fire) {
      this._onSignal(symbol, result);
    }
  }

  // A watcher's entry condition just fired. Always report it (for the
  // popup); only actually place the trade if the bot is armed.
  _onSignal(symbol, result) {
    this.onEvent("signal", {
      symbol,
  ...
  onContractSettled(contract) {
    ...
    if (this.sessionPnl <= -Math.abs(this.settings.stopLoss)) {
      this.setArmed(false);
      this.onEvent("status", { message: `Stop loss hit (${this.sessionPnl.toFixed(2)} USD). Bot disarmed.` });
      this.onEvent("armed_off", {});
      return;
    }
    if (this.sessionPnl >= Math.abs(this.settings.targetProfit)) {
      this.setArmed(false);
      this.onEvent("status", { message: `Target profit reached (+${this.sessionPnl.toFixed(2)} USD). Bot disarmed.` });
      this.onEvent("armed_off", {});
      return;
    }
    if (this.settings.maxWinStreak && this.winStreak >= this.settings.maxWinStreak) {
      this.setArmed(false);
      this.onEvent("status", {
        message: `${this.winStreak} wins in a row — pausing to re-check market conditions, as planned.`,
      });
      this.onEvent("armed_off", {});
      return;
    }
